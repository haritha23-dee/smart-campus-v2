...
import { getFacultyProfile, updateFacultyProfile, resolveFileUrl } from '../../services/facultyService';
import { DESIGNATIONS } from '../../constants/adminConstants';
...
export default function FacultyProfile() {
  ...
  useEffect(() => {
    (async () => {
      setInitLoading(true);
      try {
        const { user } = await getFacultyProfile();
        setProfile(user);
        setName(user.name || '');
        setDesignation(user.designation || DESIGNATIONS[0]);
        setYearJoined(user.yearJoined || '');
        setSubjectsCanTeach((user.subjectsCanTeach || []).join(', '));
  ...
  const handleSubmit = async (e) => {
    ...
    setMsg({ text: '', type: '' });
    try {
      const { user: updated } = await updateFacultyProfile({
        name: name.trim(),
        designation,
        yearJoined,
        subjectsCanTeach,
      ...
      <form onSubmit={handleSubmit} className="bg-surface border border-border-subtle rounded-2xl p-6 space-y-6">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Comma separated, e.g. DBMS, Operating Systems"
            className="w-full px-4 py-2.5 text-sm rounded-xl bg-canvas border border-border-subtle focus:outline-none focus:ring-2 focus:ring-brand"
          />
          <p className="text-[11px] text-neutral-500 dark:text-neutral-400 mt-1.5">Reference only — not enforced when posting resources.</p>
        </div>
