...
import { getLabHistory } from '../../services/labStaffService';
...
export default function LabStaffHome() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const { history } = await getLabHistory();
        setMetrics({
          pendingCount: history.pendingCount || 0,
          issuedCount: history.issuedHistory?.length || 0,
          overdueCount: history.overdueEquipment?.length || 0,
        });
      } catch {
        //not hard-fail on a metrics error
      } finally {
        setLoading(false);
      }
    })();
    ...
    <div className="max-w-5xl mx-auto px-10 py-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
        {[
          ...
          { label: 'Currently Issued Units', value: metrics.issuedCount },
          { label: 'Overdue Equipment', value: metrics.overdueCount },
        ].map((m) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Link
          to="/lab-staff/inventory"
          className="border border-border-subtle rounded-2xl bg-surface p-6 hover:shadow-lg transition-all"
        >
          <h2 className="text-lg font-semibold mb-2">Equipment Inventory</h2>
          <p className="text-sm text-neutral-600 dark:text-neutral-400">
            Add, edit, and manage lab equipment stock by section.
          </p>
        </Link>

        <Link
          to="/lab-staff/requests"
          className="border border-border-subtle rounded-2xl bg-surface p-6 hover:shadow-lg transition-all"
        >
          <h2 className="text-lg font-semibold mb-2">Booking Requests</h2>
          <p className="text-sm text-neutral-600 dark:text-neutral-400">
            Review pending equipment requests and assign due dates.
          </p>
        </Link>

        <Link
          to="/lab-staff/returns"
          className="border border-border-subtle rounded-2xl bg-surface p-6 hover:shadow-lg transition-all"
        >
          <h2 className="text-lg font-semibold mb-2">Return Tracking</h2>
          <p className="text-sm text-neutral-600 dark:text-neutral-400">
            Track active bookings and mark equipment as returned.
          </p>
        </Link>
      </div>
    </div>
