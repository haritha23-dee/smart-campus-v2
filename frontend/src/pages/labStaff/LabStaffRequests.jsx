...
import { listBookingRequests, approveBookingRequest, rejectBookingRequest } from '../../services/labStaffService';
...
export default function LabStaffRequests() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { requests: r } = await listBookingRequests('pending');
      setRequests(r || []);
    } catch (err) {
  ...
  const submitApprove = async (e) => {
    ...
    setApproveError('');
    try {
      await approveBookingRequest(approveTarget._id, loanDays);
  ...
  const confirmReject = async () => {
    ...
    setRejecting(true);
    try {
      await rejectBookingRequest(rejectTarget._id);
  ...
  return (
    <div className="space-y-6">
          ...
          {requests.map((r) => (
            <div key={r._id} className="border border-border-subtle rounded-xl p-4 bg-surface flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="shrink-0 w-11 h-11 rounded-lg bg-neutral-200 dark:bg-neutral-800 flex items-center justify-center">
                  <Wrench className="w-5 h-5 text-neutral-500 dark:text-neutral-400" />
                </div>
                <div>
                  <p className="text-sm font-semibold">{r.equipment?.name}</p>
      ...
      <Modal open={!!approveTarget} onClose={() => setApproveTarget(null)} title="Approve Request">
        <form onSubmit={submitApprove} className="space-y-4">
          ...
          <p className="text-xs text-neutral-500 dark:text-neutral-400">
            Approving <span className="font-semibold text-neutral-700 dark:text-neutral-200">{approveTarget?.equipment?.name}</span> for {approveTarget?.student?.name}.
          </p>
          <div>
      ...
      <ConfirmDialog
        ...
        message={rejectTarget ? `"${rejectTarget.equipment?.name}" for ${rejectTarget.student?.name} will be rejected.` : ''}
        confirmLabel="Reject"
        danger
        loading={rejecting}
