...
import { listLabReturns, markEquipmentReturned } from '../../services/labStaffService';
import ConfirmDialog from '../../components/common/ConfirmDialog';
import Toast from '../../components/common/Toast';

export default function LabStaffReturns() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { list: l } = await listLabReturns();
      setList(l || []);
    } catch (err) {
  ...
  const confirmReturn = async () => {
    ...
    setMarking(true);
    try {
      await markEquipmentReturned(returnTarget._id);
      setToast({ type: 'success', message: 'Marked as returned — units restored.' });
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        message={returnTarget ? `"${returnTarget.equipment?.name}" will be marked returned and unit availability restored.` : ''}
