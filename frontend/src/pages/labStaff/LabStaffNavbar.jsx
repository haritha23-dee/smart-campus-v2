...
export default function LabStaffNavbar() {
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between px-10 py-5 border-b border-border-subtle bg-canvas/80 backdrop-blur-md">
      <div className="flex items-center gap-3">
        ...
        >
          Smart Campus Resource Portal — Lab Staff
        </Link>
        {user?.department?.name && (
          <span className="text-[10px] uppercase tracking-wide px-2 py-1 rounded bg-brand/10 text-brand font-semibold">
            {user.department.name}
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="relative" ref={menuRef}>
          ...
          >
            ...
            <span className="text-xs font-medium normal-case tracking-normal">{user?.name || 'Lab Staff'}</span>
          ...
          {profileMenuOpen && (
            <div className="absolute right-0 mt-2 w-48 rounded-xl border border-border-subtle bg-surface shadow-xl py-1.5 z-50">
              <button
                onClick={() => { setProfileMenuOpen(false); navigate('/lab-staff'); }}
              ...
              <button
                onClick={() => { setProfileMenuOpen(false); navigate('/lab-staff/history'); }}
                className="w-full flex items-center gap-2.5 px-4 py-2 text-xs font-medium hover:bg-neutral-200/50 dark:hover:bg-neutral-800/50 transition-colors text-left"
              >
              ...
              <button
                onClick={() => { setProfileMenuOpen(false); navigate('/lab-staff/profile'); }}
                className="w-full flex items-center gap-2.5 px-4 py-2 text-xs font-medium hover:bg-neutral-200/50 dark:hover:bg-neutral-800/50 transition-colors text-left"
              >
