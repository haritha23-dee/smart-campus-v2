...
export default function LabStaffHistory() {
  const [history, setHistory] = useState({ issuedHistory: [], returnedHistory: [], overdueEquipment: [], pendingCount: 0 });
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const { history: h } = await getLabHistory();
        setHistory(h);
      } catch {
  ...
  const rows = tab === 'issued' ? history.issuedHistory : tab === 'returned' ? history.returnedHistory : history.overdueEquipment;
          ...
          {rows.map((r) => (
            <div key={r._id} className="border border-border-subtle rounded-xl p-4 bg-surface flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="shrink-0 w-10 h-10 rounded-lg bg-neutral-200 dark:bg-neutral-800 flex items-center justify-center">
