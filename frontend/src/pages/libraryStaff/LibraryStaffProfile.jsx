...
import { getLibraryProfile, updateLibraryProfile, changePassword, resolveFileUrl } from '../../services/libraryStaffService';

export default function LibraryStaffProfile() {
  ...
  useEffect(() => {
    (async () => {
      setInitLoading(true);
      try {
        const { user } = await getLibraryProfile();
        setProfile(user);
        setName(user.name || '');
  ...
  const handleSubmit = async (e) => {
    ...
    setMsg({ text: '', type: '' });
    try {
      const { user: updated } = await updateLibraryProfile({ name: name.trim(), yearJoined, photoFile });
      setProfile(updated);
      updateUser(updated);
      setPhotoFile(null);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between pb-4 border-b border-border-subtle">
        ...
        <button
          onClick={() => navigate('/library-staff')}
